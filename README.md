...
> file: `kijito_inbox_monitor.py`. POSIX (Linux/macOS); Windows runs interval-only.
...
`--state-file ~/.cache/kijito-inbox-monitor/hive.json --persona codex --persona argus` writes `hive.codex.json` and
...
`~/.cache/kijito-inbox-monitor/events.<persona>.ndjson`.
...
(`~/.cache/kijito-inbox-monitor/events.<persona>.ndjson`) — each session subscribes to only its own mail (see Agent
Signposting). Rotation is IN-PROCESS (no `newsyslog` / `logrotate` / `sudo`, no orphaned-fd blinding). stderr
goes to `~/.cache/kijito-inbox-monitor/monitor.err`.
...
```text
~/.cache/kijito-inbox-monitor/hive.<persona>.json      # STATE: cursor/FSM bookkeeping (internal — do NOT tail this)
~/.cache/kijito-inbox-monitor/events.<persona>.ndjson  # EVENTS: the stream you tail to consume your mail
```
...
emit primitive) but ships Kijito as the reference source. Published as **Kijito Inbox Monitor** (package
`kijito-inbox-monitor`).
